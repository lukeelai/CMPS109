         ...
         try {
            ...
            switch (token.symbol) {
               case tsymbol::NUMBER:{
                  ...
                  //convert number to vector and pass to bigint
                  unsigned char currChar;
                  vector<unsigned char> newNumber;
                  for(int i = token.lexinfo.length()-1; i >=0; i--){
                     currChar = token.lexinfo.at(i);
                     newNumber.push_back(currChar);
                  }
                  operand_stack.push(bigint(newNumber));
